...
(function() {
	...
	var url = url_sendMsg;
	var state = getState();
//	var state = {id:"1",account:"admin",role:"npc"};
...
//			net: ''+plus.networkinfo.getCurrentType()
	...
	feedback.addFile = function(path) {
		//为了限制只传一张图，原来的 name:"images"+index 的  +index 去掉了。
		feedback.files.push({name:"images",path:path,id:"img-"+index});
		index++;
	};
	/**
	 * 初始化图片域占位
...
//						feedback.newPlaceholder();
	...
	feedback.submitBtn.addEventListener('tap', function(event) {
		...
		feedback.send(mui.extend({},feedback.user,{
...
//			plus.nativeUI.closeWaiting()
			console.log("upload cb:"+upload.responseText);
				//上传成功，重置表单
				if (upload.responseText == 'Success') {
//					mui.toast('反馈成功~')
					mui.toast("发送成功");
